...
impl<'a, G: GameEnv, E: Evaluator<G>> GumbelMCTS<'a, G, E> {
    ...
    pub fn run(&mut self) -> Option<MctsSearchResult> {
        ...
        if candidates.len() == 1 {
            // 只有一个候选动作，直接返回
            return self.build_result(candidates[0]);
        }
        ...
        // 4. 收集所有数据并返回
        let action = self.final_action(&remaining)?;
        self.build_result(action)
    }
    ...
    fn run_phase(&mut self, visits_per_action: usize, remaining: &[usize]) -> (usize, usize) {
        ...
        (phase_usage, terminal_hits)
    }

    /// 批量评估收集到的叶子并回填（单次 `evaluator.evaluate`）。
    fn evaluate_and_apply(&mut self, batch: &[PendingEval<G>]) {
